...
$$\hat{U}(s') = \lim_{\Delta s \to 0} \prod_{i = 1}^n \mathrm{e}^{\hat{\eta}(s_i) \Delta s}$$ {#eq:etaintegral}
where $s_i = i \Delta s$, $n = \lfloor s' / \Delta s \rfloor$, and $\lfloor x \rfloor$ denotes the floor of $x$.  This is the formal solution to the linear differential equation [@Eq:etadefinition].  The product integral in [@Eq:etaintegral] may also be reinterpreted as “$s$-ordering” [@reimann2013quantum] in analogy to time-ordering from quantum field theory.
